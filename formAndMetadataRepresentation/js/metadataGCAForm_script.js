...
function getReferenceRules( index )
{
		return [
				...
				{input: "#citationOnlineRessourceInput" + index, message: "Invalid url", action: "blur, keyup", rule: function( arguments )
				 {
					 return ("" === arguments[0].value || /^(http|https|ftp)\:\/\/[a-z0-9\.-]+\.[a-z]{2,4}/gi.test( arguments[0].value ));
				 }},
...
function createReferenceFieldset( containerIdRef, index )
{
		...
		// Mail input --> not mail, url in fact so adapted
		//$( "#citationOnlineRessourceInput" + index ).jqxInput( {height: "20px", placeHolder: "someone@mail.com"} );
		$( "#citationOnlineRessourceInput" + index ).jqxInput( {height: "20px", placeHolder: "url"} );
...
function manageFormDiv()
{
		...
		$( "#standAloneInput" ).jqxInput( {height: "20px", placeHolder: "url"} );
		$( "#originalDataUrlInput" ).jqxInput( {height: "20px", placeHolder: "url"} );
...
function sendMails( mailListTo, mailFrom, fileList, isToAdministrator )
{
		...
		if( isToAdministrator )
				messageVal = "Hi,\n\nA new medatadata description has been done : ";
		else
